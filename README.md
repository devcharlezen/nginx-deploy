...
#### First configuration
```
 nano /etc/nginx/sites-available/{folder_name eg devcharlezen}
```
```
server {
  listen 80;
  ...
  location / {
        root /var/www/{folder_name eg devcharlezen};
...
```
ln -s /etc/nginx/sites-available/netflix /etc/nginx/sites-enabled/{folder_name eg devcharlezen}

```

##### Write your fist message
```
nano /var/www/{folder_name eg devcharlezen}/index.html
...
```
location /api {
        proxy_pass http://{ip_address eg 0.0.0.0}:8000;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection 'upgrade';
        proxy_set_header Host $host;
...
```
rm -rf /var/www/{folder_name eg devcharlezen}/*
```
```
mkdir /var/www/{folder_name eg devcharlezen}/client
```

```
cp -r build/* /var/www/{folder_name eg devcharlezen}/client
```

Let's make some server configuration
```
 location / {
        root /var/www/{folder_name eg devcharlezen}/client/;
        index  index.html index.htm;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection 'upgrade';
...
server {
 listen 80;
 server_name {domain_name eg devcharlezen}.com www.{domain_name eg devcharlezen}.com;
...
server {
  listen 80;
  server_name api.{domain_name eg devcharlezen}.com;
  location / {
    proxy_pass http://{ip_address eg 0.0.0.0}:8800;
    proxy_http_version 1.1;
    proxy_set_header Upgrade $http_upgrade;
    proxy_set_header Connection 'upgrade';
    proxy_set_header Host $host;
...
server {
  listen 80;
  server_name admin.{domain_name eg devcharlezen}.com;
  location / {
    root /var/www/{folder_name eg devcharlezen}/admin;
    index  index.html index.htm;
    proxy_http_version 1.1;
    proxy_set_header Upgrade $http_upgrade;
    proxy_set_header Connection 'upgrade';
